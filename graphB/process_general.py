...
from joblib import (
    Parallel,
    delayed,
)  # os.system("pip install joblib") if joblib isn't installed
...
def get_depth_first_tree(csr_adj_matrix, node_index):
    ...
    degree_list = sorted(
        [(degree, node) for node, degree in original_adj_graph.degree()], reverse=True
    )
    ...
    for node, neighbors in original_adj_graph.adj.items():
        for neighbor, weight in neighbors.items():
            if node > neighbor:
                ...
                i += 1
    tree_rand_weights = nx.minimum_spanning_tree(
        original_adj_graph
    )  # default is kruskal's
...
# Called in get_spanning_tree
def get_random_st_adj_matrix(csr_adj_matrix, config_obj):  # BROKEN
    ...
    # dictonary of keys matrix from scipy
    dok_st_adj_matrix = dok_matrix(
        (NUM_VERTICES, NUM_VERTICES), dtype=int
    )  # create adj matrix of same size as graph
    ...
    while len(visited) < NUM_VERTICES:
        ...
        dok_st_adj_matrix[outside_edge[1], outside_edge[0]] = csr_adj_matrix[
            outside_edge[1], outside_edge[0]
        ]
        dok_st_adj_matrix[outside_edge[0], outside_edge[1]] = csr_adj_matrix[
            outside_edge[0], outside_edge[1]
        ]
...
def get_spanning_tree(
    unbalanced_csr_adj_matrix, spanning_tree_type, tree_name, rand_weights, node_index
):
    if spanning_tree_type == "random":
        tree = None
        if hasattr(unbalanced_csr_adj_matrix, "value"):
            tree = get_random_MST_adj_matrix(
                unbalanced_csr_adj_matrix.value, rand_weights[tree_name]
            )
        else:
            tree = get_random_MST_adj_matrix(
                unbalanced_csr_adj_matrix, rand_weights[tree_name]
            )
        nx_st_graph = nx.from_scipy_sparse_matrix(tree)
        assert nx.is_tree(nx_st_graph)
        return tree
    elif spanning_tree_type == "random":  ## BROKEN
...
def create_edge_path_dict(path_dict):
    ...
    for v1, v2 in [
        ...
        for v1 in path_dict
        for v2 in path_dict
        if (v1 < v2 and v2 in path_dict[v1])
    ]:
...
def balance_graph(
    ...
    print_timing_output("Begin graph balancing:", str(datetime.now()), output_type)
    changed_edge_list = []
    tree_type = get_tree_type(config_obj)
    print_timing_output("Tree type: -------------------->", tree_type, output_type)
    csr_st_adj_matrix = get_spanning_tree(
        b_unbalanced_csr_adj_matrix, tree_type, tree_name, rand_weights, node_index
    )  # types: random, random_MST, breadth, depth
    tree_time = datetime.now() - start_balance
    print_timing_output(
        "TREE_TIME: Random Tree Acquired, took: (hh:mm:ss.ms)", tree_time, output_type
    )
...
def get_non_st_edges(csr_unbal_adj_matrix, csr_st_adj_matrix):
    ...
    if hasattr(csr_unbal_adj_matrix, "value"):
        ...
        non_st_edges_list = [
            (rows[i], cols[i], csr_unbal_adj_matrix.value[rows[i], cols[i]])
        ...
        non_st_edges_list = [
            (rows[i], cols[i], csr_unbal_adj_matrix[rows[i], cols[i]])
            for i in range(len(rows))
            if cols[i] > rows[i]
        ]
...
def should_change_edge(st_graph, unique_paths, coord_triple):
    cycle_list = unique_paths[
        (coord_triple[0], coord_triple[1])
    ]  ## the list of all cycles in the graph
    cycle_sign_list = [
        st_graph[i][j]["weight"] for i, j in zip(cycle_list, cycle_list[1:])
    ]
...
def reverse_edge(csr_adj_matrix, coord_triple):
    csr_adj_matrix[coord_triple[0], coord_triple[1]] = (
        0 - csr_adj_matrix[coord_triple[0], coord_triple[1]]
    )
    csr_adj_matrix[coord_triple[1], coord_triple[0]] = (
        0 - csr_adj_matrix[coord_triple[1], coord_triple[0]]
    )
    return True
...
def write_component_stats(
    f, csr_bal_adj_matrix, csr_st_adj_matrix, component_list, config_obj
):
    ...
    if len(unique) > 1:
        ...
        component_list_with_tuples = [
            (component_list_dict[entry], c0_size_percent)
            if component_list_dict[entry] == 0
            else (component_list_dict[entry], c1_size_percent)
            for entry in range(total_nodes)
        ]
...
def write_vertex_degree_stats(f, csr_bal_adj_matrix, csr_st_adj_matrix, component_list):
    ...
    avg_deg_c0, avg_deg_c1 = get_components_avg_degree(
        balanced_graph_degree_list, component_list
    )
    ...
    num_p, num_p_0, num_p_1 = get_num_pendant_vertices(
        vertex_neighbor_amt_list, component_list
    )
...
def write_balanced_h5(
    ...
    try:
        ...
        if len(num_components) > 1:
            ...
            write_vertex_degree_stats(
                f, csr_bal_adj_matrix, csr_st_adj_matrix, component_list
            )
...
def get_unvisited_neighbors(visited, nx_st_graph):
    neighbor_lists = [
        nx_st_graph.neighbors(visited_vertex) for visited_vertex in visited
    ]
    all_neighbors = set(
        [vertex for neighbor_list in neighbor_lists for vertex in neighbor_list]
    )
...
def get_balance_components(nx_st_graph):
    ...
    while len(visited) < NUM_VERTICES:
        ...
        component_list = assign_component_numbers(
            component_list, unvisited_neighbors, visited, nx_st_graph
        )
    ...
    total_neg = (
        len(
            [
                i
                for i in range(len(rows))
                if b_csr_unbal_adj_matrix[rows[i], cols[i]] == -1
            ]
        )
        / 2
    )
    ...
    pos_to_neg = (
        len(
            [
                ...
                for i in range(len(rows))
                if b_csr_unbal_adj_matrix[rows[i], cols[i]] == 1
                and csr_bal_adj_matrix[rows[i], cols[i]] == -1
            ]
        )
        / 2
    )
...
def write_outcome_stats(f, config_obj, csr_bal_adj_matrix, component_map_list):
    ...
    matrix_file_path = get_full_h5_path(config_obj)
    outcomes = None
    try:
        full_h5 = h5py.File(
            matrix_file_path + str(config_obj["component_no"]) + ".h5", "r"
        )
    ...
    vertex_outcome_component_dict = {
        vertex: {
            "C": component_map_list[vertex],
            "O": identify_record(outcomes[vertex]),
        }
        for vertex in range(csr_bal_adj_matrix.shape[0])
    }
    ...
    try:
        ...
        print(
            "Warning: Either c0 or c1 is missing at least one winner or at least one loser. Bias score set to: -999"
        )
...
def generate_tree_names(config_obj):
    tree_names = [str(uuid.uuid4()) for i in range(config_obj["num_trees"])]
    return tree_names
